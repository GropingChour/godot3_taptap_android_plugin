...
func _create_key_section():
	...
	save_key_btn = Button.new()
	save_key_btn.text = "💾 保存密钥"
	save_key_btn.connect("pressed", self, "_on_save_key_pressed")
	key_buttons.add_child(save_key_btn)
	
	group.add_child(key_buttons)
	
	# 密钥文件路径显示
	var key_path_label = Label.new()
	key_path_label.text = "密钥文件位置: " + ProjectSettings.globalize_path(KEY_FILE_PATH_XML) + " (XML) / " + ProjectSettings.globalize_path(KEY_FILE_PATH_RAW) + " (RAW)"
...
func _on_save_key_pressed():
	...
	if _save_key_to_files(new_key):
		current_key = new_key
		status_label.text = "✅ 密钥已保存到文件"
	else:
		status_label.text = "❌ 保存密钥失败"

func _save_key_to_files(key: String) -> bool:
	"""保存密钥到 XML 和 RAW 文件"""
	var xml_success = _save_key_to_xml(key)
	var raw_success = _save_key_to_raw(key)
	return xml_success or raw_success
